...
namespace Libraries.inventory.item
{
    ...
    public class Consumable : ModelInventoryItemConsumable
    {
        ...
        public ModelInventoryInventoryItem ToInventoryItem(string key, int position, int level, int index, int count = 1, int seed = 0, string serializationData = "addsilent", string sellable = "false", string tradeable = "false", string destroyable = "false", string isBound = "false")
        {
            ...
            ModelInventoryInventoryItem Item = new ModelInventoryInventoryItem() { Id = key, Type = InventoryItemTypes.Consumable, Itempos = position, Count = count, Index = index, Sellable = sellable, Tradeable = tradeable, Destroyable = destroyable, Isbound = isBound, Level = level, Seed = seed, SerializationData = serializationData };
